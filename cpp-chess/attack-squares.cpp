...
const uint8_t knight_dr[] = {-2, -2, -1, -1,  1, 1, 2,  2};
const uint8_t knight_df[] = { 1, -1, -2,  2, -2, 2, 1, -1};

const uint8_t bishop_dirs = 4;
const uint8_t bishop_dr[] = {1,  1, -1, -1};
const uint8_t bishop_df[] = {1, -1,  1, -1};

const uint8_t rook_dirs = 4;
const uint8_t rook_dr[] = {0,  0, 1, -1};
const uint8_t rook_df[] = {1, -1, 0,  0};

const uint8_t queen_dirs = 8;
const uint8_t queen_dr[] = {0,  0, 1, -1, 1,  1, -1, -1};
const uint8_t queen_df[] = {1, -1, 0,  0, 1, -1,  1, -1};
// The King direction data is the same as queen just with max_dist = 1

const uint8_t pawn_attack_dirs = 2;
const uint8_t white_pawn_attack_dr[] = { 1, 1};
const uint8_t white_pawn_attack_df[] = {-1, 1};
const uint8_t black_pawn_attack_dr[] = {-1, -1};
const uint8_t black_pawn_attack_df[] = {-1,  1};

#define WHITE_INDEX 0
#define BLACK_INDEX 1

_AttackSquares::_AttackSquares() : 
			attack_squares(64), pawn_attack_squares(64), pawn_move_squares(64) {

	vector<uint8_t> path;

	for(int sq = 0; sq < 64; sq++) {
		// init attack_squares at index
		attack_squares[sq] = vector<vector<vector<uint8_t> > >(6);

		// knight moves
		for(int i = 0; i < knight_dirs; i++) {
			path = travel_in_dir(sq, knight_dr[i], knight_df[i]);
			if(!path.empty()) {
				attack_squares[sq][KNIGHT].push_back(path);
			}
		}
		...
		for(int i = 0; i < bishop_dirs; i++) {
			...
			if(!path.empty()) {
				attack_squares[sq][BISHOP].push_back(path);
			}
		}
		...
		for(int i = 0; i < rook_dirs; i++) {
			...
			if(!path.empty()) {
				attack_squares[sq][ROOK].push_back(path);
			}
		}
		...
		for(int i = 0; i < queen_dirs; i++) {
			...
			if(!path.empty()) {
				attack_squares[sq][QUEEN].push_back(path);
			}
		}
		...
		for(int i = 0; i < pawn_attack_dirs; i++) {
			...
			if(!path.empty()) {
				pawn_attack_squares[sq][BLACK_INDEX].push_back(path[0]);
			}
		}

		// intitalize pawn_move_squares at index
		pawn_move_squares[sq] = vector<vector<uint8_t> >(2);

		pawn_move_squares[sq][WHITE_INDEX] = travel_in_dir(sq,  1, 0, rank(sq) == 1 ? 2 : 1);
		pawn_move_squares[sq][BLACK_INDEX] = travel_in_dir(sq, -1, 0, rank(sq) == 6 ? 2 : 1);
