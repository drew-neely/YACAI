...
void run_game_end_tests(string filename) {
	...
	int num_pass = 0;
	int num_fail = 0;
	uint8_t endReason = NO_GAME_END;
	uint8_t ensuredReason = NO_GAME_END;
	for(int i = 0; i < tests.size(); i++) {
		GameEndTest test = tests[i];
		Board board;
		// check moves and get to end position
		for(size_t move_idx = 0; move_idx < test.moves.size(); move_idx++) {
			Move move = test.moves[move_idx];
			...
			board.makeMove(move);
			if(move_idx + 1 < test.moves.size() && board.state->game_end_reason != NO_GAME_END) {
				if(is_edge_case_draw(board)) {
					num_pass++;
					goto endtest;
				}
				printf("FAIL : test %d\n", i);
				printf("\tGame marked as finished before last move was applied\n");
				printf("\tfen = %s\n", board.get_fen().c_str());
				printf("\texpected reason      : %d (NO_GAME_END)\n", NO_GAME_END);
				printf("\tobserved reason      : %d\n", board.state->game_end_reason);
				num_fail++;
				goto endtest;
			}
		}
		endReason = board.state->game_end_reason;
		board.setGameEndReason();
		ensuredReason = board.state->game_end_reason;
		if(endReason == test.end_reason && ensuredReason == test.end_reason) {
			// printf("PASS : test %d\n", i);
			num_pass++;
		} else {
			printf("FAIL : test %d\n", i);
			...
			printf("\tendreason            : %d (winner : %s, reason : %s)\n", endReason, winner_str.at(winner(endReason)).c_str(), reason_str.at(reason(endReason)).c_str());
			printf("\tsetGameEndReason     : %d (winner : %s, reason : %s)\n", ensuredReason, winner_str.at(winner(ensuredReason)).c_str(), reason_str.at(reason(ensuredReason)).c_str());
			num_fail++;
			goto endtest;
		}
		endtest:;
